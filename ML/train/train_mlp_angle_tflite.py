...
def load_angle_features_csv(csv_path: Path, target_col: str) -> tuple[pd.DataFrame, pd.DataFrame, np.ndarray]:
    """
    Returns:
    ...
    if target_col not in df.columns:
        raise SystemExit(f"features.csv must contain '{target_col}' (run label_mendeley_semg.py --target angle).")

    y = df[target_col].to_numpy(dtype=np.float32, copy=False)

    # Build feature frame by dropping metadata + all possible target columns.
    # IMPORTANT: never include the target column itself as an input feature (label leakage).
...
def filter_only_subject(full_df: pd.DataFrame, x: np.ndarray, y: np.ndarray, subject: str) -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    full_df2 = full_df.loc[mask].reset_index(drop=True)
    x2 = x[mask]
    y2 = y[mask]
    return full_df2, x2, y2
...
def stratified_group_split_regression(
    ...
    *,
    ...
    """
    Group split (record/subject) while trying to preserve the y distribution.
    We bin y into n_bins and then greedily assign groups to match global bin counts.
    """
    rng = np.random.default_rng(seed)
    ...
    rng.shuffle(group_list)  # random tie-breaks among similar sizes
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train MLP regressor for elbow angle and export TFLite.")
    parser.add_argument(
        "--features-csv",
        type=str,
        default="ML/datasets/Mendeley/sEMG_recordings/normalized/labeled/features.csv",
        help="features.csv produced by label_mendeley_semg.py --target angle",
    )
    parser.add_argument("--target-col", choices=["angle_norm", "angle_deg"], default="angle_norm")
    parser.add_argument("--out-dir", type=str, default="ML/models/mlp_angle", help="Output directory")
    parser.add_argument("--val-frac", type=float, default=0.2)
    parser.add_argument("--test-frac", type=float, default=0.1)
    parser.add_argument("--split-by", choices=["row", "record", "subject"], default="subject")
    parser.add_argument(
        "--only-subject",
        type=str,
        default=None,
        help="Restrict training/eval to a single subject (e.g. 'subject_1' or 'Subject_1').",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="0,1",
        help='Clip range for stratification, e.g. "0,1" for angle_norm or "0,150" for angle_deg.',
    )
    ...
    full_df, xdf, y = load_angle_features_csv(features_csv, str(args.target_col))
    ...
    # Output: angle_norm in [0,1] -> sigmoid; angle_deg -> linear
    out_act = "sigmoid" if str(args.target_col) == "angle_norm" else "linear"
    layers.append(tf.keras.layers.Dense(1, activation=out_act, name="angle_out"))
    ...
    meta = {
        ...
        "base_feature_columns": list(xdf.columns),
